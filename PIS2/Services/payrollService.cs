...
namespace PIS2.Services {
    ...
    public class PayrollService
    {
        ...
        public async Task<DateTime> GetLastPayroll(int emp) 
        {
            ...
            return lastPay.payrollModel.EndDate;
        }
        public async Task GeneratePayrollAsync(int payrollId, string modifiedBy)
        {
            ...
            var employeesQuery = _db.Employments.Include(e => e.employmentTypeModel)
                .Where(e => e.employmentStatus == mainStatus.Active).AsQueryable();
            ...
            var allAbsences = await _db.LeaveHistoryView.AsNoTracking()
                .Where(l => l.leaveStatus == leaveStatus.Posted && l.leaveGroup == leaveGroup.Absentism && l.leaveHistoryAction == leaveStatus.Posted
            ...
            var AllCredit = _db.Persons.Include(p => p.SubAccount).AsNoTracking().ToList();
            ...
            try
            {
                ...
                foreach (var emp in employees)
                {
                    ...
                    var pay = await CalculateEmployeePayAsync(
                        ...
                        taxRates, otherDeds, credit ?? 0, debit
                        );
                    ...
                    var pensione = pension * pensionEmployer / pensionEmployee;
        ...
        public async Task<payrollPay> CalculateEmployeePayAsync(
            ...
            int pensionDedType, 
            DateTime? empLastPay,
            jobPlacementModel jobp,
            decimal absences,
            decimal overtimeAmount,
            int OtEarningType,
            List<earningModel> empEarnings,
            List<allowanceAssignmentModel> allowances,
            int AllowanceEarningType,
            int SalaryEarningType,
            int TaxDeductionType,
            List<taxRateModel> taxRates,
            List<deductionModel> otherDeds,
            ...
            )
        {
            ...
            if (empType.isPensionAllowed)
            {
                ...
                {
                    deductionTypeID = pensionDedType,
            ...
            foreach (var od in otherDeds.OrderBy(d => d.DeductionType?.deductionPriority))
            {
                ...
                deductions.Add(new deductionRecordModel
                {
                    ...
                    deductionReference = od.DeductionType?.deductionCode ?? "other",
                    modifiedBy = "system"
                });
            }
            ...
            if (debit == 0) throw new InvalidOperationException($"Debit Sub AccountID ={debit}!.");
        ...
        public async Task CompletePayrollAsync(int payrollId, string modifiedBy)
        {
            ...
            if (_db.JournalEntries.Any(j => j.Reference == $"Payroll-{payrollId}"))
            ...
            var pays = await _db.PayrollPays
            ...
            var allAbsences = await _db.Leaves.AsNoTracking()
                .Where(l => absRange.Contains(l.leaveID) && l.leaveStatus == leaveStatus.Posted && l.leaveTypeModel.leaveGroup == leaveGroup.Absentism && empIds.Contains(l.employmentID))
                .ToListAsync();
            ...
            try
            {
                ...
                var debitAccounts = _db.SubAccounts.Include(s => s.accountModel).Where(p => subAccounts.Contains(p.subAccountID))
                    .ToDictionary(s=>s.subAccountID, s => s.accountModel.accountID);
                var empCreditAccount = _db.Accounts.FirstOrDefault(a => a.accountName == EMPLOYEE_CREDIT_ACCOUNT_NAME);
                ...
                var defaultSubAccount = await _db.SubAccounts.FirstOrDefaultAsync(s => s.subAccountName.ToLower() == defaultSubAccName.ToLower());
                ...
                foreach (var p in pays)
                {
                    ...
                    foreach (var d in p.DeductionRecords)
                    {
