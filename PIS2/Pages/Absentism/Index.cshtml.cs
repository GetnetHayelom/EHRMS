...
namespace PIS2.Pages.Absentism
{
    ...
    public class IndexModel : PageModel
    {
        ...
        {
            var absLeaveIDs = _context.LeaveTypes.Where(l => l.leaveGroup == leaveGroup.Absentism).Select(l => l.leaveTypeID).ToList();
            var leaves= _context.LeaveReportView.Where(l => absLeaveIDs.Contains(l.LeaveTypeID ?? 0) && (l.LeaveStart >= StartDate && l.LeaveEnd <= EndDate)).AsQueryable();
            ...
            leaveModel = await leaves.ToListAsync();

            var twelveMonthsAgo = DateTime.Now.AddMonths(-12);

            MonthlyTrend = leaveModel
                .Where(l => l.LeaveStart >= twelveMonthsAgo)
                .GroupBy(l => new
                {
            ...
            YearlyTrend = await _context.LeaveReportView
                .Where(l => l.LeaveGroup == leaveGroup.Absentism)
                .GroupBy(l => new
                {
