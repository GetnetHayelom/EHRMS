...
public class PayrollService
{
    ...
    public async Task GeneratePayrollAsync(int payrollId, string modifiedBy)
    {
        ...
        var employeesQuery = _db.Employments.Where(e => e.employmentStatus ==mainStatus.Active).AsQueryable();
        ...
        foreach (var emp in employees)
        {
            ...
            totalNet += pay.NetPay;
            totalTax += pay.deductions.Where(d => d.DeductionType != null && d.DeductionType.deductionName.ToLower().Contains("tax")).Sum(d => d.deductionAmount ?? 0);
            totalPenEmp += pay.deductions.Where(d => d.DeductionType != null && d.DeductionType.deductionName.ToLower().Contains("pension") && d.DeductionType.isMandatory).Sum(d => d.deductionAmount ?? 0);
    ...
    public async Task<payrollPay> CalculateEmployeePayAsync(employmentModel emp, payrollModel payroll)
    {
        ...
        decimal baseSalary = emp.JobPlacements.FirstOrDefault(j => j.jobPlacementStatus== mainStatus.Active).jobPlacementSalary; // assumes monthly
        ...
        int workingDaysInPeriod = await GetWorkingDaysInPeriod(start, end, null);
        ...
        decimal basePay = 0m;
        if (emp.employmentTypeModel.employmentTypeName == "Permanent")
        {
            basePay = baseSalary * (workedHours / totalPossibleHours);
        }
        else
        {
        ...
        decimal gross = basePay + overtimeAmount + allowancesSum;

        // Compute pension employee (deduct before tax)
        decimal pensionEmployee = Math.Round(baseSalary * 0.07M, 2, MidpointRounding.AwayFromZero);//OR FROM BASE PAY##############################
        decimal pensionEmployer = Math.Round(baseSalary * 0.11M, 2, MidpointRounding.AwayFromZero);//######################################
        ...
        if (taxRate != null)
        {
            tax = Math.Max(0,Math.Round((taxable * taxRate.taxRate) - taxRate.deduction, 2));
            if (tax < 0) tax = 0;
        }
        ...
        foreach (var od in otherDeds.OrderBy(d => d.DeductionType?.dedcutionPriority))
        {
            ...
            deductions.Add(new deductionModel
            {
                ...
                DeductionType = od.DeductionType,
                deductionAmount = od.deductionAmount,
                employmentID = od.employmentID,
                deductionStatus = mainStatus.Active,
                modifiedBy = "system"
            });
        }
        ...
        var payrollPay = new payrollPay
        {
            ...
            earnings = new List<earningModel> {
                new earningModel { earningAmount = basePay, earningTypeId = 1, earningReference = 0, employmentID = emp.employmentID, earningStatus = mainStatus.Active, modifiedBy = "system" },
                new earningModel { earningAmount = overtimeAmount, earningTypeId = 2, earningReference = 0, employmentID = emp.employmentID, earningStatus = mainStatus.Active, modifiedBy = "system" }
            },
        ...
        //########################################
    private async Task<int> GetWorkingDaysInPeriod(DateTime start, DateTime end, int? companyId)
    {
